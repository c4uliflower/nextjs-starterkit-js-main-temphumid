...
// │      → GET /api/limits/p1f2/per-sensor
...
// │      → GET /api/sensor-readings/p1f2/current
...
// │      → POST /api/limits/p1f2/per-sensor
...
// [BACKEND #1] → GET /api/limits/p1f2/per-sensor
const DEFAULT_SENSOR_LIMITS = {
  //                    tempUL  tempLL  humidUL  humidLL
  "brother-assy-1": { tempUL: 28, tempLL: 20, humidUL: 70, humidLL: 40 },
  "brother-assy-2": { tempUL: 28, tempLL: 20, humidUL: 70, humidLL: 40 },
  "jcm-pcba":       { tempUL: 28, tempLL: 20, humidUL: 70, humidLL: 40 },
...
function SensorLimitsContent({ allLimits, onSave, onClose, sensors }) {
  ...
  // [BACKEND #4] → POST /api/limits/p1f2/per-sensor
  const handleSaveLimits = async () => {
    ...
    try {
      ...
      // const res  = await fetch("/api/limits/2/per-sensor", {
      //   method:  "POST",
  ...
  return (
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
      ...
      <div style={{ padding: "16px 20px 14px", borderBottom: "1px solid #e9ecef", flexShrink: 0 }}>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Plant 1 Floor 2 · Each sensor has its own threshold</p>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "auto" }}>
        <div style={{ padding: "14px 24px" }} className="bg-background">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {totalActiveCount} sensor{totalActiveCount !== 1 ? "s" : ""} active · Plant 1 Floor 2
          </p>
        </div>
