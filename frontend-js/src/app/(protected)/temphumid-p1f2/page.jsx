...
// │      → Poll every ~10s
// │
// │  #2  handleSaveLimits() — POST /api/limits/p1f2/per-sensor
// │      → Payload: { sensors: [{ areaId, tempUL, tempLL, humidUL, humidLL }] }
// │
// │  #3  activeLocation flag
// │      → GET /api/locations/p1f2/active-status
...
// Example: const INACTIVE_AREAS = new Set(["P1F2-01"]);
...
// Sensor metadata — id, name, color, x, y, direction stay hardcoded (layout config).
// areaId links each sensor to the DB registry (Temp_Logger_Chip_ID.Area ID).
// temp, humid, hasData, status, limits — all come from the API via useEffect.
const MAP_SENSORS = [
  ...
  useEffect(() => {
    ...
    const fetchReadings = async () => {
      try {
        ...
        console.error("Failed to fetch P1F2 readings:", err);
      }
    };
