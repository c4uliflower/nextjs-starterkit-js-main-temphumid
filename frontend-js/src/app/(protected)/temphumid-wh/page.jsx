...
// │      → GET /api/sensor-readings/wh/current
// │      → Poll every ~10s
// │
// │  #2  handleSaveLimits() — POST /api/limits/wh/per-sensor
// │      → Payload: { sensors: [{ areaId, tempUL, tempLL, humidUL, humidLL }] }
// │
// │  #3  activeLocation flag
// │      → GET /api/locations/wh/active-status
...
function SensorLimitsContent({ allLimits, onSave, onClose, sensors }) {
  const [draft,    setDraft]    = useState(() =>
    Object.fromEntries(
      ...
      sensors.map(({ id }) => [id, { ...(allLimits[id] ?? { tempUL: 40, tempLL: 5, humidUL: 70, humidLL: 20 }) }])
    )
  );
  ...
  useEffect(() => {
    ...
    const fetchReadings = async () => {
      try {
        ...
        console.error("Failed to fetch WH readings:", err);
      }
    };
