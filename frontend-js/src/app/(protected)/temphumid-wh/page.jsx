...
// Verified from Temp_Logger_Chip_ID (Plant='2', Floor='WH', Location='P2F1WH')
const MAP_SENSORS = [
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
      <div style={{ padding: "16px 20px 14px", borderBottom: "1px solid #e9ecef", flexShrink: 0 }}>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Warehouse · Each sensor has its own threshold
          {hasChanges && (
...
// Only sensors with actual changes are sent to the API on save.
function SensorStatusContent({ onClose, sensors, onSaved }) {  // 🆕 added onSaved prop
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
      ...
      <div style={{ padding: "16px 20px 14px", borderBottom: "1px solid #e9ecef", flexShrink: 0 }}>
        ...
        <p className="text-sm text-muted-foreground mt-0.5" style={{ display: "flex", alignItems: "center", gap: 8 }}>
          Warehouse · {activeCount} active, {inactiveCount} inactive
          {hasChanges && !saving && (
...
export default function WHMapPage() {
  ...
  useEffect(() => {
    ...
    const fetchReadings = async () => {
      try {
        const res  = await axios.get(`${API_BASE}/sensors/readings/current`, { params: { floor: "wh" } });
        const json = res.data;
        ...
        if (Object.keys(statusCache).length === 0) {
          try {
            const res     = await axios.get(`${API_BASE}/sensors/status`, { params: { floor: "wh" } });
      ...
      } catch (err) { console.error("Failed to fetch WH readings:", err); }
    };
