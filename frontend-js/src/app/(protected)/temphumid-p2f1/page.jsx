...
// │      → GET /api/limits/p2f1/per-sensor
...
// │      → POST /api/limits/p2f1/per-sensor
...
// [BACKEND #1] → GET /api/limits/p2f1/per-sensor
const DEFAULT_SENSOR_LIMITS = {
  //                     tempUL  tempLL  humidUL  humidLL
  "fg":               { tempUL: 30, tempLL: 15, humidUL: 85, humidLL: 35 },
  "warehouse-office": { tempUL: 30, tempLL: 15, humidUL: 85, humidLL: 35 },
  "wh-cs":            { tempUL: 10, tempLL:  0, humidUL: 70, humidLL:  0 },
  "wh-cs2":           { tempUL: 10, tempLL:  0, humidUL: 70, humidLL:  0 },
  "wo-north":         { tempUL: 30, tempLL: 15, humidUL: 85, humidLL: 35 },
  "wo-south-ha":      { tempUL: 30, tempLL: 15, humidUL: 85, humidLL: 35 },
  "wo-sw-iqc":        { tempUL: 30, tempLL: 15, humidUL: 85, humidLL: 35 },
...
function SensorLimitsContent({ allLimits, onSave, onClose, sensors }) {
  ...
  // [BACKEND #4] → POST /api/limits/p2f1/per-sensor
  const handleSaveLimits = async () => {
    ...
    try {
      ...
      // const res  = await fetch("/api/limits/p2f1/per-sensor", {
      //   method:  "POST",
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "auto" }}>
        <div style={{ padding: "14px 24px" }} className="bg-background">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {totalActiveCount} sensor{totalActiveCount !== 1 ? "s" : ""} active · Plant 2 Floor 1
          </p>
        </div>

        <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 12, padding: 20, overflow: "visible" }}>
          <div style={{ flex: 1, position: "relative", overflow: "visible", borderRadius: 5, background: "transparent", border: "1px solid #e9ecef" }}>
            <img
